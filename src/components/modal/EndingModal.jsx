...
export default function EndingModal({ stage, endingData, handleRestart, onCollection, collectedCount, disableEffects, collectionComplete }) {
  ...
  const dialogue = typeof jobEpilogue === 'string' ? jobEpilogue : jobEpilogue.dialogue ?? '';
  const description = typeof jobEpilogue === 'string' ? '' : jobEpilogue.description ?? '';
  const { count: dialogueCount, done: dialogueDone, finish: finishDialogue } = useTypewriter(dialogue, revealed && done);
  const { count: descriptionCount, done: descriptionDone, finish: finishDescription } = useTypewriter(description, revealed && done && dialogueDone);
  ...
  const allTextDone = done && dialogueDone && descriptionDone;
  return (
      ...
      {!revealed ? <div className="ending-transition" role="status" aria-live="polite"><span>CONNECTION LOST</span><i aria-hidden="true" /></div> : <div className="ending-layout">
        ...
        <section className="ending-copy">
          ...
          <div className="ending-prose">
            ...
            {done && dialogue && <div className="ending-job-afterword">
              <p className="ending-job-dialogue"><TypedText text={dialogue} count={dialogueCount} /></p>
              {dialogueDone && description && <p className="ending-job-description"><TypedText text={description} count={descriptionCount} /></p>}
            </div>}
          </div>
