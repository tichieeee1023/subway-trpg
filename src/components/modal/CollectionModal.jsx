...
export default function CollectionModal({ collected, onClose, onSecretComplete }) {
  ...
  return <DialogFrame title={secret ? '히든 후일담' : `엔딩 도감 · ${collected.length}/7`} onClose={onClose} className="collection-dialog">
    ...
    })}</div><button className={`utility-done${complete ? ' collection-secret-unlocked' : ''}`} disabled={!complete} onClick={() => setSecret(true)}>{complete ? '✦ 04:44 AM — 히든 후일담 열기' : `히든 후일담 잠김 · ${collected.length}/7 수집`}</button></>}
  </DialogFrame>;
}
