...
export default function CollectionModal({ collected, onClose, onSecretComplete }) {
  ...
  return <DialogFrame title={secret ? '히든 후일담' : `엔딩 도감 · ${collected.length}/6`} onClose={onClose} className="collection-dialog">
    {secret ? <SecretReport onBack={onSecretComplete} /> : <><p>완료한 엔딩이 이 브라우저에 기록됩니다.</p><div className="collection-grid">{Object.values(ENDING_DEFINITIONS).map((ending) => <article key={ending.id}>{collected.includes(ending.id) ? <><img src={ENDING_CARDS[ending.cardId]} alt={ending.title} /><h3>{ending.title}</h3></> : <div className="locked-card"><span aria-hidden="true">?</span><p>미발견 엔딩</p></div>}</article>)}</div><button className={`utility-done${complete ? ' collection-secret-unlocked' : ''}`} disabled={!complete} onClick={() => setSecret(true)}>{complete ? '✦ 04:44 AM — 히든 후일담 열기' : `히든 후일담 잠김 · ${collected.length}/6 수집`}</button></>}
  </DialogFrame>;
}
